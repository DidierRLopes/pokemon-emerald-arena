...
test('success reveals the actual download and next steps, then recovers from a wrong file',async()=>{
  ...
  await ui.select();assert.equal(calls,1);assert.equal(ui.nodes.ready.hidden,false);assert.equal(ui.nodes.download.download,'Emerald-Arena-0.3.1.gba');assert.equal(ui.nodes.download.focused,true);
