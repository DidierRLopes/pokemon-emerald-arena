...
test('success reveals the actual download and next steps, then recovers from a wrong file',async()=>{
  ...
  await ui.select();assert.equal(calls,1);assert.equal(ui.nodes.ready.hidden,false);assert.equal(ui.nodes.download.download,'Emerald-Arena-0.5.0.gba');assert.equal(ui.nodes.download.focused,true);
...
test('README puts a direct ZIP before gameplay and never calls a video Play',async()=>{
  ...
  assert.match(md,/releases\/download\/v0\.5\.0\/Emerald-Arena-0\.5\.0\.zip/);
